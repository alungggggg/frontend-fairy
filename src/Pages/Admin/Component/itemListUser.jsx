...
const ItemListUser = ({ items, getUser }) => {
    ...
    if (items.length === 0) {
        return <tr><td colSpan="5" className="text-center">User Tidak Ada</td></tr>;
    }
        ...
        <>
            {items.map((item) => (
                <tr key={item.id}>
                    ...
                    <td>
                        <button className="btn btn-sm btn-success border me-2" onClick={() => updateUser(item.id)}>Update</button>
                        <button className="btn btn-sm btn-danger border" onClick={() => deleteUser(item.id)}>Hapus</button>
                    </td>
                </tr>
            ))}
        </>
